...
//	String[] strGrid={"7","8","9","/","4","5","6","*","1","2","3","-" ,"0","C","+","=" };
	...
	String[] strGrid={"7","8","9","+","4","5","6","-","1","2","3","*" ,"0","=","%","/" }; // 계산기 기호변경  ---- %추가
	...
	public NewCalculator(){
		frame=new JFrame("계산기예제"); // 타이틀 "계산기예제"로 변경
...
//		frame.setJMenuBar(menuBar);  				// 메뉴바 붙이기(제거)
		
		frame.setResizable(false);                                  //창 크기 변경 못하게 막는다.
		
		
		//frame 크기 변경
//		frame.setSize(220,310);        //기존 frame 크기 
	...
	public void actionPerformed(ActionEvent e){
		...
		try{	
			...
			if(str!="/" && str!= "*"&& str!= "-"&& str!= "+"&& str!= "C"&& str!= "=" && str!="%"){ // 연산기호 %추가	
			...
			//연산기호가 들어왔을때 true.
			if(str=="/" || str== "*"|| str== "-"|| str== "+"|| str== "=" || str== "%"){ // 연산자 %추가
			...
			//모든 값 초기화 시키고 다시 연산. --- 초기화 안되는부분 확인
			if(str=="C"){
				first="";
	...
	{
		NewCalculator cal=new NewCalculator(); //클래스명 변경
		cal.gui();
	}// main()
}// class
